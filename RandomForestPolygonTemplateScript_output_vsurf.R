...
ModelDataset = cbind(ModelDataset,as.factor(ReferencePoints[,2]))
...
if (facts != '')
{
	for(i in 1:length(facts))
	{
		for(j in 1:dim(Apply_ZonalStats)[2])
		{
			if(colnames(Apply_ZonalStats)[j]==facts[i])
			{
				factlevels = c(levels(Apply_ZonalStats[,j]),levels(ModelDataset[,j-1]))
				levels(ModelDataset[,j-1])=factlevels
				levels(Apply_ZonalStats[,j])=factlevels
				rm(factlevels)
				gc()
			}	
		}
...
vsurf1 = VSURF(ModelDataset[,1:second2Last],ModelDataset[,lastColumn],data =ModelDataset,parallel=TRUE,ncores=4)
usePredsLimited = unlist(vsurf1[2])
usePredsLeast = unlist(vsurf1[3])


#Use all predictors
RF_Model1 = randomForest(ModelDataset[,1:second2Last],ModelDataset[,lastColumn], data=ModelDataset,importance=TRUE,ntree=1000)
print("USING ALL VARIABLES")
print(RF_Model1)

#Use limited predictors
RF_Model2 = randomForest(ModelDataset[,usePredsLimited],ModelDataset[,lastColumn], data=ModelDataset,importance=TRUE,ntree=1000)
print("USING ALL VARIABLES THAT ARE IMPORTANT")
print(RF_Model2)

#Use very few predictors
RF_Model3 = randomForest(ModelDataset[,usePredsLeast],ModelDataset[,lastColumn], data=ModelDataset,importance=TRUE,ntree=1000)
