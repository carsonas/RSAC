...
#OutputModel = "I:/Ethiopia/Change_Detection/change_test.img"
#
###############################################################################################################################################################
...
install.packages("VSURF", repos='http://cran.us.r-project.org')
...
lastColumn = dim(ModelDataset)[2]
second2Last = lastColumn - 1
print("Calculating VSURF")
vsurf1 = VSURF(ModelDataset[,2:lastColumn],ModelDataset[,1],data =ModelDataset,parallel=TRUE,ncores=4)
usePredsLimited = unlist(vsurf1[2]) + 1
usePredsLeast = unlist(vsurf1[3]) + 1

#Use all predictors
RF_Model1 = randomForest(ModelDataset[,2:lastColumn],ModelDataset[,1], data=ModelDataset,importance=TRUE,ntree=1000)
print("USING ALL VARIABLES")
print(RF_Model1)

#Use limited predictors
RF_Model2 = randomForest(ModelDataset[,usePredsLimited],ModelDataset[,1], data=ModelDataset,importance=TRUE,ntree=1000)
print("USING ALL VARIABLES THAT ARE IMPORTANT")
print(RF_Model2)

#Use very few predictors
RF_Model3 = randomForest(ModelDataset[,usePredsLeast],ModelDataset[,1], data=ModelDataset,importance=TRUE,ntree=1000)
print("USING ALL VARIABLES THAT ARE IMPORTANT AND REMOVING REDUNDANCY")
print(RF_Model3)
...
if(predicttype=="Thematic")
	...
	OOBs = function(inRF)
		{
		accClass1 = as.data.frame(inRF[5])[1,3]
		accClass2 = as.data.frame(inRF[5])[2,3]
	...
	OOBvec = c(OOB1,OOB2,OOB3)
	whichOOB = which.min(OOBvec)
	...
	OOB1 = tail(RF_Model1$mse,n=1)
	OOB2 = tail(RF_Model2$mse,n=1)
	OOB3 = tail(RF_Model3$mse,n=1)
...
if(predicttype=="Thematic")
{
	...
	legendout = paste(base,"_legend.csv",sep="",collapse = "")
	write.csv(legend, legendout,row.names=FALSE)
	rm(legendout)
	rm(confusion_matrix)
	rm(output_confusion)
	gc()
...
varimp = paste(base,"_VarImpPlot.jpg",sep="",collapse = "")
png(filename=varimp)
...
if(predicttype=="Thematic"){
	...
	outputrast = predict(ourStack, RF_Model, filename = OutputModel, type='response',progress = "text", datatype = 'FLT4S', inf.rm = TRUE)
}
