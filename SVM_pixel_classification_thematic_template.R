...
SVM_Model = ksvm(Class~.,data = ModelDataset,type="C-svc")
print(SVM_Model)
...
if(predicttype=="Thematic"){
	outputrast = predict(ourStack, SVM_Model, filename = OutputModel, type='response',progress = "text", datatype = 'INT1U', inf.rm = TRUE)
}else{
	outputrast = predict(ourStack, SVM_Model, filename = OutputModel, type='response',progress = "text", datatype = 'FLT4S', inf.rm = TRUE)
}
